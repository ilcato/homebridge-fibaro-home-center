...
export class SetFunctions {
	...
	async setTargetHeatingCoolingState(value, context, characteristic, service, IDs) {
	  ...
	  await this.command('setThermostatMode', [v], service, IDs);	// ... and full mode on mode controller
	}
	...
	async setTargetTemperature(value, context, characteristic, service, IDs) {
	  ...
	  await this.command('setInterval',
	  	[parseInt(this.platform.config.thermostattimeout) + Math.trunc((new Date()).getTime() / 1000)], service, IDs);
	}
